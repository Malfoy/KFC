...
inline char const* sfx(char const* txt) {
	return txt;
}
#else
inline char const* sfx(char const*) {
	return "";
}
#endif
...
#if lest_FEATURE_WSTRING
inline std::string to_string(wchar_t const* const txt) {
	return txt ? to_string(std::wstring(txt)) : "{null string}";
}
inline std::string to_string(wchar_t* const txt) {
	return txt ? to_string(std::wstring(txt)) : "{null string}";
}
#endif
...
inline std::string to_string(signed char chr) {
	return to_string(static_cast<char>(chr));
}
inline std::string to_string(unsigned char chr) {
	return to_string(static_cast<char>(chr));
}
...
#if lest_FEATURE_WSTRING
inline auto to_string(std::wstring const& txt) -> std::string {
	std::string result;
...
inline auto make_memory_string(void const* item, std::size_t size) -> std::string {
...
inline auto to_string(approx const& appr) -> std::string {
...
inline text red(text words) {
	return "\033[1;31m" + words + "\033[0m";
}
inline text green(text words) {
	return "\033[1;32m" + words + "\033[0m";
}
inline text gray(text words) {
...
inline bool is_cout(std::ostream& os) {
	return &os == &std::cout;
}
...
inline std::ostream& operator<<(std::ostream& os, colourise words) {
	return words(os);
}
#else
inline text colourise(text words) {
	return words;
}
#endif
...
inline int indefinite(int repeat) {
	return repeat == -1;
}
...
inline int run(tests specification, texts arguments, std::ostream& os = std::cout) {
	try {
		...
		if (option.count) {
			return for_test(specification, in, count(os));
		}
		if (option.list) {
			return for_test(specification, in, print(os));
		}
		if (option.tags) {
			return for_test(specification, in, ptags(os));
		}
		if (option.time) {
			return for_test(specification, in, times(os, option));
		}
