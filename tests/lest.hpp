...
#if defined(__clang__)
#	define lest_SUPPRESS_WSHADOW _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wshadow\"")
#	define lest_SUPPRESS_WUNUSED _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wunused-value\"")
#	define lest_RESTORE_WARNINGS _Pragma("clang diagnostic pop")

#elif defined(__GNUC__)
#	define lest_SUPPRESS_WSHADOW _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wshadow\"")
#	define lest_SUPPRESS_WUNUSED _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wunused-value\"")
...
#	define lest_CASE(specification, proposition)                                                                                                              \
		...
		namespace {                                                                                                                                            \
		lest::add_test lest_REGISTRAR(specification, lest::test(proposition, lest_FUNCTION));                                                                  \
		}                                                                                                                                                      \
		static void lest_FUNCTION(lest::env& lest_env)

#else // lest_FEATURE_AUTO_REGISTER

#	define lest_CASE(proposition) proposition, [](lest::env & lest_env)

#	define lest_CASE_ON(proposition, ...) proposition, [__VA_ARGS__](lest::env & lest_env)

#	define lest_MODULE(specification, module)                                                                                                                 \
		namespace {                                                                                                                                            \
		lest::add_module _(specification, module);                                                                                                             \
		}
...
#define lest_SECTION(proposition)                                                                                                                              \
	lest_SUPPRESS_WSHADOW static int lest_UNIQUE(id) = 0;                                                                                                      \
	if (lest::guard(lest_UNIQUE(id), lest__section, lest__count))                                                                                              \
		for (int lest__section = 0, lest__count = 1; lest__section < lest__count; lest__count -= 0 == lest__section++)                                         \
			for (lest::ctx lest__ctx_section(lest_env, proposition); lest__ctx_section;) lest_RESTORE_WARNINGS
...
#define lest_EXPECT_NO_THROW(expr)                                                                                                                             \
	do {                                                                                                                                                       \
		try {                                                                                                                                                  \
			lest_SUPPRESS_WUNUSED expr;                                                                                                                        \
			lest_RESTORE_WARNINGS                                                                                                                              \
		} catch (...) {                                                                                                                                        \
			lest::inform(lest_LOCATION, #expr);                                                                                                                \
		}                                                                                                                                                      \
		if (lest_env.pass()) lest::report(lest_env.os, lest::got_none(lest_LOCATION, #expr), lest_env.context());                                              \
	} while (lest::is_false())
...
#define lest_EXPECT_THROWS(expr)                                                                                                                               \
	do {                                                                                                                                                       \
		try {                                                                                                                                                  \
			...
			if (lest_env.pass()) lest::report(lest_env.os, lest::got{ lest_LOCATION, #expr }, lest_env.context());                                             \
			break;                                                                                                                                             \
		}                                                                                                                                                      \
...
#define lest_EXPECT_THROWS_AS(expr, excpt)                                                                                                                     \
	do {                                                                                                                                                       \
		try {                                                                                                                                                  \
			...
			if (lest_env.pass()) lest::report(lest_env.os, lest::got{ lest_LOCATION, #expr, lest::of_type(#excpt) }, lest_env.context());                      \
			break;                                                                                                                                             \
		} catch (...) {}                                                                                                                                       \
...
#define lest_LOCATION                                                                                                                                          \
	lest::location { __FILE__, __LINE__ }
...
struct add_test
{
	add_test(tests& specification, test const& test_case) { specification.push_back(test_case); }
};
...
inline text
not_expr(text message)
{
	return "! ( " + message + " )";
}

inline text
with_message(text message)
{
...
inline void
inform(location where, text expr)
{
	try {
		throw;
	} catch (message const&) {
...
template<typename T>
auto
make_value_string(T const& value) -> std::string;

template<typename T>
auto
make_memory_string(T const& item) -> std::string;
...
inline std::string
to_string(std::string const& txt)
{
	return "\"" + txt + "\"";
}
#if lest_FEATURE_WSTRING
inline std::string
to_string(std::wstring const& txt);
#endif

inline std::string
to_string(char const* const txt)
{
	return txt ? to_string(std::string(txt)) : "{null string}";
}
inline std::string
to_string(char* const txt)
{
	return txt ? to_string(std::string(txt)) : "{null string}";
}
#if lest_FEATURE_WSTRING
inline std::string
to_string(wchar_t const* const txt)
{
	return txt ? to_string(std::wstring(txt)) : "{null string}";
}
inline std::string
to_string(wchar_t* const txt)
{
	return txt ? to_string(std::wstring(txt)) : "{null string}";
}
#endif
...
inline std::string
to_string(signed short value)
{
	return make_value_string(value);
}
inline std::string
to_string(unsigned short value)
{
	return make_value_string(value) + sfx("u");
}
inline std::string
to_string(signed int value)
{
	return make_value_string(value);
}
inline std::string
...
inline std::string
to_string(signed long value)
{
	return make_value_string(value) + sfx("l");
}
inline std::string
to_string(unsigned long value)
{
	return make_value_string(value) + sfx("ul");
}
inline std::string
to_string(signed long long value)
{
	return make_value_string(value) + sfx("ll");
}
inline std::string
to_string(unsigned long long value)
{
	return make_value_string(value) + sfx("ull");
}
inline std::string
...
{
	return make_value_string(value) + sfx("f");
}

inline std::string
to_string(signed char chr)
{
	return to_string(static_cast<char>(chr));
}
inline std::string
to_string(unsigned char chr)
{
	return to_string(static_cast<char>(chr));
}
...
to_string(char chr)
{
	...
	for (auto tr : table) {
		if (chr == tr.chr) return tr.str;
	}

	auto unprintable = [](char c) { return 0 <= c && c < ' '; };

	return unprintable(chr) ? to_string(static_cast<unsigned int>(chr)) : "\'" + std::string(1, chr) + "\'";
}
...
template<typename T>
auto
make_enum_string(T const& item) -> ForNonEnum<T, std::string>
{
#if lest__cpp_rtti
...
template<typename T>
auto
make_enum_string(T const& item) -> ForEnum<T, std::string>
{
...
template<typename T>
auto
make_string(T const& item) -> ForNonStreamable<T, std::string>
{
...
template<typename T>
auto
make_string(T const& item) -> ForStreamable<T, std::string>
{
...
auto
make_string(std::pair<T1, T2> const& pair) -> std::string
{
...
auto
make_string(std::tuple<TS...> const& tuple) -> std::string
{
...
template<typename T>
auto
to_string(T const* ptr) -> std::string
{
...
template<typename T>
auto
to_string(T const& item) -> ForNonContainerNonPointer<T, std::string>
{
...
template<typename C>
auto
to_string(C const& cont) -> ForContainer<C, std::string>
{
	std::ostringstream os;
	os << "{ ";
	for (auto& x : cont) { os << to_string(x) << ", "; }
	os << "}";
	return os.str();
}
...
#if lest_FEATURE_WSTRING
inline auto
to_string(std::wstring const& txt) -> std::string
{
	std::string result;
	result.reserve(txt.size());

	for (auto& chr : txt) { result += chr <= 0xff ? static_cast<char>(chr) : '?'; }
...
inline auto
make_memory_string(void const* item, std::size_t size) -> std::string
{
...
inline auto
to_string(approx const& appr) -> std::string
{
...
auto
to_string(L const& lhs, std::string op, R const& rhs) -> std::string
{
	std::ostringstream os;
...
struct expression_lhs
{
	...
	{
		return result{ lhs == rhs, to_string(lhs, "==", rhs) };
	}
	template<typename R>
	result operator!=(R const& rhs)
	{
...
red(text words)
{
	return "\033[1;31m" + words + "\033[0m";
}
inline text
green(text words)
{
	return "\033[1;32m" + words + "\033[0m";
}
inline text
gray(text words)
{
	return "\033[1;30m" + words + "\033[0m";
}

inline bool
starts_with(text words, text with)
{
	return 0 == words.find(with);
}

inline text
replace(text words, text from, text to)
{
...
colour(text words)
{
	if (starts_with(words, "failed"))
		return replace(words, "failed", red("failed"));
	else if (starts_with(words, "passed"))
		return replace(words, "passed", green("passed"));

	return replace(words, "for", gray("for"));
}

inline bool
is_cout(std::ostream& os)
{
	return &os == &std::cout;
}
...
struct colourise
{
	...
	std::ostream& operator()(std::ostream& os) const { return is_cout(os) ? os << colour(words) : os << words; }
};

inline std::ostream&
operator<<(std::ostream& os, colourise words)
{
	return words(os);
}
#else
...
inline text
pluralise(text word, int n)
{
	return n == 1 ? word : word + "s";
}

inline std::ostream&
operator<<(std::ostream& os, comment note)
{
	return os << (note ? " " + note.info : "");
}

inline std::ostream&
operator<<(std::ostream& os, location where)
{
#ifdef __GNUG__
...
inline void
report(std::ostream& os, message const& e, text test)
{
...
#if lest_FEATURE_REGEX_SEARCH
inline bool
search(text re, text line)
{
	return std::regex_search(line, std::regex(re));
}
#else
inline bool
search(text part, text line)
{
	auto case_insensitive_equal = [](char a, char b) { return tolower(a) == tolower(b); };

	return std::search(line.begin(), line.end(), part.begin(), part.end(), case_insensitive_equal) != line.end();
}
#endif

inline bool
match(texts whats, text line)
{
	for (auto& what : whats) {
		if (search(what, line)) return true;
	}
	return false;
}

inline bool
select(text name, texts include)
{
	...
	if (none(include)) { return !hidden(name); }
	...
	for (auto pos = include.rbegin(); pos != include.rend(); ++pos) {
		...
		if (search(part, name)) return true;

		if ('!' == part[0]) {
			any = true;
			if (search(part.substr(1), name)) return false;
		} else {
			any = false;
		}
	}
...
struct ptags : action
{
	...
	{
		for (auto& tag : tags(testing.name)) result.insert(tag);

		return *this;
	}

	~ptags() { std::copy(result.begin(), result.end(), std::ostream_iterator<text>(os, "\n")); }
};
...
struct count : action
{
	...
	~count() { os << n << " selected " << pluralise("test", n) << "\n"; }
};
...
struct timer
{
	...
	double elapsed_seconds() const { return 1e-6 * static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(time::now() - start).count()); }
};
...
template<typename Action>
Action&&
for_test(tests specification, texts in, Action&& perform, int n = 1)
{
	for (int i = 0; indefinite(n) || i < n; ++i) {
		for (auto& testing : specification) {
			if (select(testing.name, in))
				if (abort(perform(testing))) return std::move(perform);
		}
	}
...
inline void
shuffle(tests& specification, options option)
{
...
inline seed_t
seed(text opt, text arg)
{
	if (is_number(arg)) return static_cast<seed_t>(lest::stoi(arg));

	if (arg == "time") return static_cast<seed_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());

	throw std::runtime_error("expecting 'time' or positive number with option '" + opt + "', got '" + arg + "' (try option --help)");
}

inline int
repeat(text opt, text arg)
{
	const int num = lest::stoi(arg);

	if (indefinite(num) || num >= 0) return num;

	throw std::runtime_error("expecting '-1' or positive number with option '" + opt + "', got '" + arg + "' (try option --help)");
}

inline auto
split_option(text arg) -> std::tuple<text, text>
{
	auto pos = arg.rfind('=');

	return pos == text::npos ? std::make_tuple(arg, "") : std::make_tuple(arg.substr(0, pos), arg.substr(pos + 1));
}

inline auto
split_arguments(texts args) -> std::tuple<options, texts>
{
	options option;
	texts in;
	...
	for (auto& arg : args) {
		if (in_options) {
			...
			if (opt[0] != '-') {
				...
				in_options = false;
				continue;
			} else if (opt == "-h" || "--help" == opt) {
				option.help = true;
				continue;
			} else if (opt == "-a" || "--abort" == opt) {
				option.abort = true;
				continue;
			} else if (opt == "-c" || "--count" == opt) {
				option.count = true;
				continue;
			} else if (opt == "-g" || "--list-tags" == opt) {
				option.tags = true;
				continue;
			} else if (opt == "-l" || "--list-tests" == opt) {
				option.list = true;
				continue;
			} else if (opt == "-t" || "--time" == opt) {
				option.time = true;
				continue;
			} else if (opt == "-p" || "--pass" == opt) {
				option.pass = true;
				continue;
			} else if (opt == "-v" || "--verbose" == opt) {
				option.verbose = true;
				continue;
			} else if ("--version" == opt) {
				option.version = true;
				continue;
			} else if (opt == "--order" && "declared" == val) { /* by definition */
				;
				continue;
			} else if (opt == "--order" && "lexical" == val) {
				option.lexical = true;
				continue;
			} else if (opt == "--order" && "random" == val) {
				option.random = true;
				continue;
			} else if (opt == "--random-seed") {
				option.seed = seed("--random-seed", val);
				continue;
			} else if (opt == "--repeat") {
				option.repeat = repeat("--repeat", val);
				continue;
			} else
				throw std::runtime_error("unrecognised option '" + arg + "' (try option --help)");
		}
		in.push_back(arg);
	}
...
inline int
run(tests specification, texts arguments, std::ostream& os = std::cout)
{
	try {
		options option;
		texts in;
...
inline int
run(tests specification, int argc, char* argv[], std::ostream& os = std::cout)
{
...
template<std::size_t N>
int
run(test const (&specification)[N], texts arguments, std::ostream& os = std::cout)
{
...
template<std::size_t N>
int
run(test const (&specification)[N], std::ostream& os = std::cout)
{
...
template<std::size_t N>
int
run(test const (&specification)[N], int argc, char* argv[], std::ostream& os = std::cout)
{
