...
int64_t kmer_in_super_kmer2(const SKC& super_kmer,const kmer_full& kmer){
	...
	__uint128_t sk_shift = super_kmer.sk >> 48;

	// Load and repeat the first and last byte of the kmer into 2 mm_128 registers
	const __m128i first_letter = _mm_set1_epi8((char)(kmer.kmer_s));
  const __m128i last_letter  = _mm_set1_epi8((char)(kmer.kmer_s >> 48));
  ...
  const __m128i sk_shift_reg  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&sk_shift));
  ...
  while (mask != 0) {
    ...
    if (kmer.kmer_s == ((sk >> shift) & 0x3FFFFFFFFFFFFFFF)) {
      return pos;
    }
