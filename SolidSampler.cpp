 ...
 */
void
SolidSampler::insert(uint8_t* kmer, std::size_t len)
{
	if (!this->alive) { throw "SolidSample previously cleaned"; }
	...
	if (!already_inserted) {
		...
		for (uint n = 0; n < NUM_HASH; n++) this->saved[positions[n]] = true;
	}
}
 ...
 */
vector<uint64_t>*
SolidSampler::get_kmers()
{
	return this->kmers;
}

ostream&
operator<<(ostream& out, SolidSampler& sampler)
{
	out << *(sampler.m_cbf);

	// Print all the kmers only if they are less than 100
	if (sampler.m_nb_kmers_saved < 100) {
		for (uint64_t i = 0; i < sampler.m_nb_kmers_saved; i++) { out << (*(sampler.kmers))[i] << ' '; }
		out << endl;
	}
	return out;
}
