...
void
BloomFilter::add(const uint8_t* data, std::size_t len)
{
	...
	if ((this->m_bits_set * 1. / m_bits.size()) >= this->m_reset_ratio) { reset(); }
}

uint64_t
BloomFilter::nbBitsSet() const
{
...
BloomFilter::possiblyContains(const uint8_t* data, std::size_t len) const
{
	...
	for (int n = 0; n < this->m_num_hashes; n++) {
		if (!m_bits.get(nthHash(n, hash_values[0], hash_values[1], m_bits.size()))) { return false; }
	}
...
ostream&
operator<<(ostream& out, BloomFilter& bf)
{
	if (bf.m_bits.size() < 10000) {
		for (uint64_t i = 0; i < bf.m_bits.size(); i++) { out << (bf.m_bits.get(i) ? 1 : 0); }
	}
	out << " (" << bf.m_bits_set << ')';
