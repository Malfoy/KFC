...
CascadingBloomFilter::CascadingBloomFilter(uint64_t size, uint8_t num_blooms, float reset_ratio)
{
	...
	for (uint i = 1; i < num_blooms; i++) this->filters[i] = new BloomFilter((size / 2) / (num_blooms - 1), optimal_nb_hash, reset_ratio);
...
{
	for (uint i = 0; i < this->m_num_blooms; i++) { delete this->filters[i]; }
}
 ...
 */
bool
CascadingBloomFilter::insert(const uint8_t* data, std::size_t len)
{
...
ostream&
operator<<(ostream& out, CascadingBloomFilter& cbf)
{
	for (BloomFilter* bf : cbf.filters) { out << *bf << endl; }
