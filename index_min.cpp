...
uint32_t
get_minimizer(uint64_t k)
{
	return k + global_kmer_size + global_minimizer_size;
}

bool
compare_minimizer(const uint64_t& a, const uint64_t& b)
{
...
void
index_full::insert(uint64_t kmer)
{
...
void
index_full::insert_seq(const string& seq)
{
	uint64_t hash = 0;
...
void
index_full::print_kmer(uint64_t num)
{
	uint64_t anc(1);
	anc <<= (2 * (kmer_size));
...
void
index_full::clear(bool force)
{
	//~ return;
	if (weak_kmer_buffer.size() > 10 * 1000 or force) {
		for (uint i(0); i < weak_kmer_buffer.size(); ++i) { dump_weak << weak_kmer_buffer[i]; }
...
void
index_min::insert(uint64_t kmer)
{
...
void
index_min::dump_counting()
{
	for (uint i(0); i < Index.size(); ++i) { Index[i].dump_counting(); }
}
